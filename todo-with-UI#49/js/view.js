import { list, statuses, priorities, id, addTask, deleteTask, changeTask  } from './main.js'
...
function createBlockTask(id_task,task_name,task_container){
    ...
    checkboxForTask.addEventListener('click', function(event){changeStatusElement(event)})
    ...
    buttonDeleteTask.addEventListener('click',function(event){deleteTaskElement(event)})
...
function changeStatusElement(event){
    ...
    if(event.target.checked){
        event.target.previousElementSibling.style.background = "#989898"
        task_status = "Done"
    }
    else{
